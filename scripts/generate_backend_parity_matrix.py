...
records the generator invocation, a generated-at UTC timestamp and
the short git SHA the matrix was regenerated against so reviewers
can spot manual edits and tell at a glance when the matrix was
last refreshed. Both header lines are stripped by ``--check`` before
comparing the rendered output to the committed copy; the rest of
the file is byte-identical between runs against the same tree.
...
def _render_markdown(cells: list[Cell]) -> str:
    ...
    # Stamp a generated-at timestamp and the current git commit short-SHA
    # at the top so reviewers can tell at a glance when the matrix was
    # last refreshed and which tree it was regenerated against. The
    # timestamp + SHA lines are stripped by ``_strip_timestamp_line`` in
    # ``--check`` mode so a fresh checkout of the same source tree
    # always compares equal to itself (the rest of the markdown is
    ...
    except Exception:  # noqa: BLE001 - git may not be available in CI sandbox
        commit_sha = "unknown"
    ...
    return (
        ...
        f">\n"
        f"> Generated by `scripts/generate_backend_parity_matrix.py` at **{generated_at} UTC** against git commit `{commit_sha}`.\n"
        f">\n"

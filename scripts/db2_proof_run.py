#!/usr/bin/env python3
"""Reproducible proof-of-port harness for the MNEMOS → IBM Db2 12.1.x port.

Connects to a target Db2 12.1.x instance via the DSN passed on the
command line (or ``DB2_PROOF_DSN`` env var), exercises every relevant
:class:`Db2Backend` repository surface, captures the SQL fingerprint
plus each result, and emits a signed JSON artifact::

    docs/proof/db2-proof-YYYYMMDD-HHMMSS.json

The artifact contains:
- Db2 version string (``sysproc.env_get_inst_info``)
- Git HEAD SHA of the repository the script ran from
- ``ibm_db`` client version
- Timestamps for the whole run + per-method timings
- Pass/fail per probe with raw return shapes (truncated)
- HMAC-SHA256 signature over the artifact's evidence body
...
``proof-<uuid>`` id prefix and cleans them up before exit.
"""
...
DEFAULT_DSN = os.environ.get(
    "DB2_PROOF_DSN",
    "db2://db2inst1:mnemos_dev@192.168.207.67:50001/MNEMOS",
)
...
async def _run(dsn: str, hmac_key: str) -> dict[str, Any]:
    from mnemos.persistence.db2 import (  # local import
        Db2Backend,
        Db2BranchRepository,
    ...
    try:
        async with pool.acquire() as conn:
            # ---- 0. Identity probes ------------------------------------
            cur = conn.cursor()
            await cur.execute("SELECT service_level FROM TABLE(sysproc.env_get_inst_info())")
            (db2_version,) = await cur.fetchone()

            dim = 768
            await cur.execute(
                "SELECT VECTOR_DISTANCE(VECTOR(:a, "
                + str(dim)
                + ", FLOAT32), VECTOR(:b, "
                + str(dim)
                + ", FLOAT32), COSINE) FROM dual",
                {
                    "a": "[" + ",".join(str(v) for v in ([1.0] + [0.0] * (dim - 1))) + "]",
                    "b": "[" + ",".join(str(v) for v in ([0.0, 1.0] + [0.0] * (dim - 2))) + "]",
                },
            )
            ...
            # ---- 1. MemoryRepository ----------------------------------
            mem = Db2MemoryRepository()

            p = Probe(
                "memory.gather_stats",
                "Db2MemoryRepository.gather_stats — aggregate-only, no Oracle tokens",
            )
            try:
            ...
            p = Probe(
                ...
                "ROOT_BYPASS list with total count — exercises :name→? bind translation",
            )
            try:
            ...
            p = Probe(
                ...
                "Round-trip by-id fetch with full column set",
            )
            try:
            ...
            p = Probe(
                ...
                "End-to-end CRUD — exercises TIMESTAMP WITH TIME ZONE and SYSTIMESTAMP translation",
            )
            try:
            ...
            # ---- 3. Semantic search (Db2 VECTOR) ---------------------
            p = Probe(
                "memory.semantic_search",
                "Db2 VECTOR_DISTANCE COSINE ranking on unit-axis embeddings (768-dim schema)",
            )
            try:
                vec_ids: list[str] = []
                dim = 768

                def _make_axis(axis_idx: int) -> list[float]:
                    v = [0.0] * dim
                    v[axis_idx] = 1.0
                    return v
                ...
                # Query embeddings: slight tilt toward x / y axes
                base_x = [0.0] * dim
                base_x[0] = 0.9
                base_x[1] = 0.1
                base_y = [0.0] * dim
            ...
            kg = Db2KGRepository()
            ver = Db2VersionRepository()
            branch = Db2BranchRepository()
            ...
            try:
                ...
                await cur.execute("DELETE FROM kg_triples WHERE id = ?", (kg_id,))
                await cur.execute("DELETE FROM memories WHERE id = ?", (anchor_mid,))
                await cur.close()
            except Exception as e:
                p.fail(e)
            probes.append(p)
            ...
            # ---- 5. State KV -----------------------------------------
            state = Db2StateRepository()
            p = Probe(
                "state.set_get_delete",
                "MERGE upsert + soft-delete — exercises SYSTIMESTAMP → CURRENT TIMESTAMP",
            )
            try:
            ...
            fed = Db2FederationRepository()
            p = Probe(
                "federation.full_lifecycle",
                "create \u2192 update \u2192 upsert \u2192 sync_log \u2192 delete on federation_peers",
            )
            try:
            ...
            try:
                ...
                await cur.execute(
                    ...
                    (
                        sub_id,
                        "https://proof.test/hook",
                        json.dumps(["memory.created"]),
                        "proof-sekret",
                        f"proof-{run_id}",
                        f"proof-{run_id}",
                    ),
                ...
                await cur.execute("DELETE FROM webhook_subscriptions WHERE id = ?", (sub_id,))
                await cur.close()
            ...
            audit = Db2ConsultationAuditRepository()
            p = Probe(
                "audit.safe_defaults",
            ...
            backend = Db2Backend(pool, None)
            p = Probe(
                "backend.transactional",
                "Db2Backend.transactional yields a usable Transaction handle",
            )
            try:
    ...
    body = {
        ...
        "ibm_db_version": ibm_db.__version__,
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=str(
            ...
            / "docs"
            / "proof"
            / f"db2-proof-{_dt.datetime.now(_dt.timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"
        ),
    )
    args = ap.parse_args()

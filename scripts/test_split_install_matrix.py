...
(pantheon / knemon / graeae) can be ADDED INDIVIDUALLY on top of it,
...
def check_scenario(name, py, result, want_addons: set[str], expect_broken=False) -> dict:
    ...
    if isinstance(routes, list):
        ...
        # CHARON is first-party core and its MPF routes must always mount.
        has_import = any(p.startswith("/v1/import") or p.startswith("/v1/export") for p in routes)
        chk("mpf_route_in_core", has_import)
    return out

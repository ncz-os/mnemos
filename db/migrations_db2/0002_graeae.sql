...
BEGIN
  ...
  EXECUTE IMMEDIATE '
    CREATE TABLE graeae_consultations (
      ...
      context_quality_summary CLOB(1M) CHECK (context_quality_summary IS JSON FORMAT JSON STRICT),
      compression_manifest CLOB(1M) CHECK (compression_manifest IS JSON FORMAT JSON STRICT),
      context_memory_ids CLOB(1M) CHECK (context_memory_ids IS JSON FORMAT JSON STRICT),
      ...
      model_variants CLOB(1M) CHECK (model_variants IS JSON FORMAT JSON STRICT),

...
-- Idempotent seed: only insert rows not already present (re-runnable --all).
-- Row 1 carries explicit CASTs so Db2 can type the derived table (a leading
-- NULL column is otherwise untyped); rows 2-12 inherit the types.
...
SELECT v.provider, v.plan_name, v.auth_method, v.monthly_usd, v.msg_cap,
       v.msg_window_seconds, v.token_cap, v.token_window_seconds, v.reset_anchor,
       v.overage_pricing_per_mtok_in, v.overage_pricing_per_mtok_out, v.notes
FROM (VALUES
  (CAST('anthropic' AS VARCHAR(128)), CAST('claude_max_200' AS VARCHAR(128)), CAST('subscription' AS VARCHAR(32)), CAST(200 AS DECIMAL(12,2)), CAST(900 AS DECIMAL(20,0)), CAST(18000 AS DECIMAL(20,0)), CAST(NULL AS DECIMAL(20,0)), CAST(NULL AS DECIMAL(20,0)), CAST('rolling' AS VARCHAR(32)), CAST(NULL AS DECIMAL(18,6)), CAST(NULL AS DECIMAL(18,6)), CAST('Claude Max 200: 900 messages per 5h window' AS VARCHAR(4000))),
...
) AS v(provider, plan_name, auth_method, monthly_usd, msg_cap, msg_window_seconds,
       token_cap, token_window_seconds, reset_anchor, overage_pricing_per_mtok_in,
       overage_pricing_per_mtok_out, notes)
WHERE NOT EXISTS (
  SELECT 1 FROM subscription_plans s
  WHERE s.provider = v.provider AND s.plan_name = v.plan_name
);

...
def test_v0_1_envelope_dump_via_helper_strips_record_v0_2_fields():
    ...
    for absent in ("provenance", "valid_time_start", "valid_time_end", "transaction_time"):
        assert absent not in record_dump, f"v0.1 record dump must not carry {absent}"
...
def _load_v0_2_schema():
    ...
    vendored = Path(__file__).resolve().parents[1] / "mnemos" / "domain" / "portability" / "vendor" / "mpf-v0.2.json"
...
def test_deletion_log_413_message_documents_pagination_unavailable():
    ...
    assert "defensive fallback" in detail.lower() or "deletion_log_next_cursor" in detail
    ...
    ), "deletion_log 413 must explicitly state that category/limit narrowing doesn't help"
...
def _build_envelope_with_morpheus_summary(in_scope_ids, source_ids):
    ...
    extra_records = [_memory_to_record({**_BASE_ROW, "id": rid}, mpf_version="0.2.0") for rid in in_scope_ids]
    ...
    for rec in records:
        if rec.provenance and "wasInfluencedBy" in rec.provenance:
            filtered = [
                ...
                if not (infl.get("type") == "memory" and infl.get("id") not in in_scope)
            ]
            if filtered:
...
def test_export_filter_logic_in_export_module_is_present():
    ...
    # Record filtering moved into the shared _iter_page_records /
    # _scope_record_provenance helpers when the per-record streaming path
    # started reusing them; the guard still asserts it lives in export.py.
    src = inspect.getsource(export_mod._iter_page_records) + inspect.getsource(export_mod._scope_record_provenance)
...
def test_export_memories_threads_deletion_log_time_window_params():
    ...
    src = inspect.getsource(export_mod._build_export_sidecars)
...
def test_cursor_always_packs_window_fields_for_self_containment():
    ...
    decoded_payload = json.loads(base64.urlsafe_b64decode(token + "=" * (-len(token) % 4)))
    ...
    decoded_unbounded = json.loads(base64.urlsafe_b64decode(token_unbounded + "=" * (-len(token_unbounded) % 4)))
...
def test_export_uses_cursor_as_sole_source_for_window_on_subsequent_pages():
    ...
    src = inspect.getsource(export_mod._resolve_deletion_log_cursor)
...
def test_export_rejects_forged_cursor_for_non_root_with_403():
    ...
    async def run():
        # No backend/tx needed — should reject before touching the DB.
...
def test_export_rejects_legacy_scopeless_cursor_with_400():
    ...
    with pytest.raises(HTTPException) as exc_info:
        asyncio.run(run())
    assert exc_info.value.status_code == 400
    assert "pre-round-4" in exc_info.value.detail or "lacks tenant-scope binding" in exc_info.value.detail
...
def test_export_documents_late_commit_caveat_with_mitigation():
    ...
    assert "quiesce" in doc.lower() or "back-to-back" in doc.lower() or "cross-check" in doc.lower()

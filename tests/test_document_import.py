...
class TestDocumentImportEndpoints:
    ...
    @patch("mnemos.api.routes.document_import.DOCLING_AVAILABLE", True)
    async def test_import_document_empty_file(self, client):
    ...
    async def test_import_document_success(self, mock_importer_class, client):
    ...
    @patch("mnemos.api.routes.document_import.DOCLING_AVAILABLE", True)
    @patch("mnemos.api.routes.document_import.DoclingImporter")
    async def test_batch_import_documents(self, mock_importer_class, client):
        ...
        mock_importer.parse_document.return_value = (
            "Content",
            {"page_count": 1},
            [{"chunk_num": 0, "title": "Test", "content": "Text", "metadata": {}}],
        )
        mock_importer_class.return_value = mock_importer
...
"""
        ...
        with patch("mnemos.api.routes.document_import.DocumentConverter") as mock_converter_class:
            ...
            importer = DoclingImporter()
            full_text, metadata, chunks = importer.parse_document(b"mock content", "test.md")

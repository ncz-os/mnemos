...
MCP_TOOLS = REPO_ROOT / "mnemos" / "mcp" / "tools"


def _extract_mcp_paths() -> list[str]:
    """Pull every literal/f-string path passed to _rest_* in
    mnemos/mcp/tools/. Returns the static prefix up to the first f-string hole."""
    src = "\n".join(
...
class TestMCPWireContract:
    ...
    def test_every_mcp_path_is_a_real_route(self):
        ...
        assert mcp_paths, "mnemos/mcp/tools exposes no paths - something is wrong with extraction"
        ...
        assert not missing, (
            f"mnemos/mcp/tools calls these paths that the REST app does not serve: {missing}. "
    ...
    def test_memory_paths_are_v1_prefixed(self):
        """Explicit regression: every memory-related path in mnemos/mcp/tools
        must carry the /v1 prefix. Reverting the prefix would reintroduce
        the v3.0.0 bug."""
        mcp_paths = _extract_mcp_paths()
        memory_paths = [p for p in mcp_paths if "/memor" in p]
        assert memory_paths, "mnemos/mcp/tools has no memory paths - extraction regex is broken"

        unprefixed = [p for p in memory_paths if not p.startswith("/v1/")]
        assert not unprefixed, (
            f"These memory paths in mnemos/mcp/tools are missing the /v1 prefix: {unprefixed}. "
    ...
    def test_kg_paths_are_v1_prefixed(self):
        ...
        unversioned /kg/* path in mnemos/mcp/tools would 404 against the
        current API."""
        mcp_paths = _extract_mcp_paths()
        kg_paths = [p for p in mcp_paths if "/kg/" in p or p.endswith("/kg")]
        assert kg_paths, "mnemos/mcp/tools has no KG paths - extraction regex is broken"
        ...
        assert not unprefixed, (
            f"These KG paths in mnemos/mcp/tools are missing the /v1 prefix: "

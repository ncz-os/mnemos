...
class _TrackingTx:
    """Transaction double that records how savepoint scopes exited.

    Nested scopes used to be `conn.transaction()`; they are now
    `tx.savepoint()`. These tests assert on WHETHER a nested scope was opened
    and how it unwound, so the recorder moved with the mechanism.
    """

    def __init__(self, conn=None, *, forbid_savepoint=False):
        self.conn = conn
        self.savepoint_exits = []
        self.savepoints_opened = 0
        self._forbid = forbid_savepoint
    ...
    def savepoint(self):
        ...
        class _Ctx:
            ...
            async def __aexit__(self_, exc_type, exc, tb):
                parent.savepoint_exits.append(exc_type)
                return False

        return _Ctx()


def _direct_backend(conn):
    """Backend + transaction pair for calling the domain import directly.

    Tests that bypass the route still need the (backend, tx) pair the domain
    function now takes, without installing anything into lifecycle.
    """
    from tests._charon_fake_backend import ConnBackedBackend, _FakeTx

    backend = ConnBackedBackend(conn)
    return backend, _FakeTx(conn)


def _install(monkeypatch, conn):
    """Install ``conn`` as the active persistence backend.

    charon's portability path no longer takes a driver connection; it takes a
    persistence backend and a transaction. ``ConnBackedBackend`` presents the
    ABC surface over this suite's existing mock connection, so the
    ``routed_rows`` substring routing and the ``fetch_calls`` recorder keep
    working exactly as before -- see tests/_fake_backend.py.
    """
...
def test_export_filters_by_caller_owner_and_namespace_for_non_root(monkeypatch):
    ...
    sql, args = next((sql, args) for sql, args in conn.fetch_calls if "owner_id = $" in sql)
...
def test_export_fetches_and_emits_distinct_verbatim_content(monkeypatch):
    # The export projection itself carries verbatim_content on all six
    # backends now, so active content and original evidence stay distinct
    # without the extra Postgres-only backfill query this test used to drive
    # (that query, and the routed_rows entry that fed it, are both gone).
    memory = _memory_row(content="compressed summary")
    memory["verbatim_content"] = "full original evidence"
    conn = _Conn(rows=[memory])
    _install(monkeypatch, conn)
...
def test_export_runs_in_repeatable_read_readonly_transaction(monkeypatch):
    ...
    backend = _install(monkeypatch, conn)
    ...
    # The snapshot is established by backend.transactional(...) now -- the
    # route owns it, and export_memories no longer opens one of its own. The
    # INVARIANT under test is unchanged and still the important one: the
    # export must run at repeatable_read, readonly.
    assert backend.last_isolation == "repeatable_read", (
        f"export must use repeatable_read isolation, got {backend.last_isolation!r}"
    )
    assert backend.last_readonly is True, f"export transaction must be readonly, got {backend.last_readonly!r}"
...
def test_import_rejects_unimplemented_published_envelope_fields_before_writes(monkeypatch, field, value):
    conn = _Conn()
    _install(monkeypatch, conn)
    env = _envelope([_memory_record()])
...
def test_import_does_not_replace_customer_metadata_with_v02_bridge(monkeypatch):
    ...
    record.payload["metadata"] = {"_mnemos_charon_mpf_v0_2_record_fields": "customer-owned-value"}
    ...
    assert metadata["_mnemos_charon_mpf_v0_2_record_fields"] == "customer-owned-value"
    ...
    assert metadata["_mnemos_charon_mpf_v0_2_record_fields_imported_envelope"] == {
        "provenance": {
            "wasAttributedTo": {"type": "user", "id": "alice"},
            "wasGeneratedBy": {"type": "activity", "id": "job-1"},
            "generatedAtTime": "2026-08-10T00:00:00Z",
...
def test_import_rejects_invalid_permission_and_quality_values(monkeypatch, field, value, message):
    conn = _Conn()
    _install(monkeypatch, conn)
    record = _memory_record()
...
def test_import_enabled_audit_chain_failure_aborts_memory_import(monkeypatch):
    ...
    settings = type("Settings", (), {"server": type("Server", (), {"session_secret": "secret"})()})()
    ...
    from tests._charon_fake_backend import ConnBackedBackend

    backend = ConnBackedBackend(conn)
    backend.audit_chain = object()
    tx = _TrackingTx(conn)
    ...
    # The audit savepoint unwound with the failure rather than swallowing it.
    assert RuntimeError in tx.savepoint_exits
...
def test_import_audit_disabled_paths_do_not_open_savepoint(monkeypatch, has_chain, enabled, secret):
    from mnemos.domain.portability import import_ as import_domain
    import mnemos.audit as audit_module
    import mnemos.core.config as config_module
    import mnemos.workers.audit_sealer as audit_sealer_module
    ...
    asyncio.run(
        import_domain._write_mpf_import_audit_entry(
            ...
            _TrackingTx(forbid_savepoint=True),
            memory_id="mem_1",
            content="body",
            category="solutions",
            subcategory=None,
...
def test_import_audit_enabled_without_secret_refuses_to_write(monkeypatch):
    ...
    async def _unexpected_write(*args, **kwargs):
        raise AssertionError("refused-write path must NOT call write_audit_entry")

    settings = type("Settings", (), {"server": type("Server", (), {"session_secret": ""})()})()
    monkeypatch.setattr(audit_module, "write_audit_entry", _unexpected_write)
    monkeypatch.setattr(config_module, "get_settings", lambda: settings)
    monkeypatch.setattr(audit_sealer_module, "audit_chain_enabled", lambda: True)
...
def test_import_idempotent_collision_compares_distinct_verbatim_from_projection(monkeypatch):
    ...
    # fetch_memory_by_id's projection now includes verbatim_content on every
    # backend, so the stored value arrives with the row instead of via a
    # second, Postgres-only "SELECT verbatim_content FROM memories" query.
    existing["verbatim_content"] = "full original evidence"
    conn = _DupeConn(routed_rows={"SELECT content, category": [existing]})
    _install(monkeypatch, conn)
    ...
    assert stats.failed == 0
    assert stats.skipped == 1
    # The distinct verbatim value still participates in the conflict
    # comparison -- it just comes from the row projection now. Assert the
    # extra round-trip is GONE rather than that it happened.
    assert not any("SELECT verbatim_content FROM memories" in sql for sql, _ in conn.fetch_calls), (
        "the Postgres-only verbatim backfill query should no longer be issued"
    )
...
def test_import_rejects_conflict_on_stale_permission_mode(monkeypatch):
    ...
    assert stats.failed >= 1
    assert any("permission_mode" in err for err in stats.errors), (
...
def _allowlist_row(memory_id="mem_alice1", owner_id="alice", namespace="alice-ns"):
    ...
    Default uses verbatim "mem_alice1" since CHARON v0.2 restricts
    memory_versions sidecar imports to the root + preserve_owner=true
    path, where envelope ids are kept as-is. Tests exercising
    non-root records-only id rewrite pass
    `memory_id=_ALICE_MEM_ALICE1_DERIVED` explicitly."""
...
def test_import_memory_versions_sidecar_imports(monkeypatch):
    ...
    # CHARON v0.2 restricts memory_versions sidecar import to root +
...
def test_import_compression_manifest_sidecar_imports(monkeypatch):
    ...
    args = cm_insert[1]
    # Non-root: records-loop derived memory_id; sidecar remap
    # translated record_id to the derived id before INSERT.
...
def test_import_kg_triple_referencing_foreign_memory_id_rejected(monkeypatch):
    ...
    assert stats.sidecars_imported == {}
    assert stats.sidecars_failed == {"kg_triples": 1}
    assert any("mem_bob_secret" in e and "not in caller-owned" in e for e in stats.errors), (
...
def test_import_preserve_owner_rejects_owner_namespace_mismatch(monkeypatch):
    ...
    assert stats.sidecars_failed == {"kg_triples": 1}
    assert any("mem_carol_real" in e and "carol" in e.lower() for e in stats.errors), (
...
def test_import_memory_versions_restores_branch_head(monkeypatch):
    ...
    assert len(branch_inserts) == 1, (
        f"expected one memory_branches UPSERT, got {len(branch_inserts)}: {[e[0][:60] for e in conn.executes]}"
    )
...
def test_import_memory_versions_rejects_cross_tenant_parent(monkeypatch):
    ...
    assert stats.sidecars_failed == {"memory_versions": 1}
    assert any("foreign-tenant" in e or "foreign-record" in e for e in stats.errors), (
...
def test_import_memory_versions_prefetches_all_parents_once(monkeypatch):
    ...
    parent_fetches = [sql for sql, _ in conn.fetch_calls if "FROM memory_versions WHERE id = ANY" in sql]
...
def test_import_memory_versions_rejects_shadowed_parent(monkeypatch):
    ...
    assert insert_args == [], (
        f"child must not insert when its parent shadows a foreign UUID; found {len(insert_args)} INSERTs"
    )
...
def test_import_non_root_memory_versions_sidecar_rejected_403(monkeypatch):
    """CHARON v0.2 architectural restriction (Codex Path B): the
    trigger-suppressed memory_versions sidecar import path is only
    available under root + preserve_owner=true. Non-root callers
    submitting a memory_versions sidecar get a 403 with a pointer
    to the documented --preserve-metadata path. Closes the entire
    round-16-through-24 class of stale-state findings structurally.
    """
    conn = _Conn()
    _install(monkeypatch, conn)
...
def test_import_non_root_record_id_is_caller_scoped_not_envelope_verbatim(monkeypatch):
    ...
    assert "mem_bob_secret" not in args, (
        f"non-root insert must NOT use envelope id verbatim; args contained 'mem_bob_secret'"
    )

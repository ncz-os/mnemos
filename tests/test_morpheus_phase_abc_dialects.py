...
def test_postgres_keeps_native_jsonb_arrays_and_conflict_upsert():
    source = "\n".join(inspect.getsource(getattr(PostgresMorpheusRepository, name)) for name in _PHASE_METHODS)
...
def test_sqlite_uses_json1_expanded_params_and_conflict_upsert():
    source = "\n".join(inspect.getsource(getattr(SqliteMorpheusRepository, name)) for name in _PHASE_METHODS)
...
def test_mysql_and_mariadb_share_phase_dialect_except_longtext_json_store():
    mysql_source = "\n".join(inspect.getsource(getattr(MysqlMorpheusRepository, name)) for name in _PHASE_METHODS)
    assert "JSON_CONTAINS_PATH" in mysql_source
    assert "JSON_SET" in mysql_source
    assert "ON DUPLICATE KEY UPDATE" in mysql_source
    assert "CAST(%s AS JSON)" in inspect.getsource(MysqlMorpheusRepository.phase_synthesise_store)
    assert MariadbMorpheusRepository.phase_consolidate is MysqlMorpheusRepository.phase_consolidate
    assert MariadbMorpheusRepository.phase_extract_store is MysqlMorpheusRepository.phase_extract_store
    assert "CAST(%s AS JSON)" not in inspect.getsource(MariadbMorpheusRepository.phase_synthesise_store)
...
class _MysqlConsolidateCursor:
    ...
    async def execute(self, sql, params):
        ...
        if normalized.startswith("SELECT config, cluster_min_size"):
            self._one = (
                json.dumps({"clusters": [{"member_memory_ids": ["canonical", "member"]}]}),
                2,
                None,
            )
        elif normalized.startswith("SELECT id, owner_id, namespace"):
            self.description = [(name,) for name in ("id", "owner_id", "namespace")]
            self._all = [("canonical", "alice", "alice"), ("member", "alice", "alice")]
            ...
            self.connection.metadata["pre_consolidate_permission_mode"] = (
                original_mode_bind if original_mode_bind is not None else self.connection.permission_mode
            )
            self.rowcount = 1
...
def test_oracle_merge_and_db2_inheritance_keep_rmw_json_fallback():
    oracle_source = "\n".join(inspect.getsource(getattr(OracleMorpheusRepository, name)) for name in _PHASE_METHODS)
...
def test_non_postgres_phase_parity_migrations_cover_retry_state(dialect):
    ...
    migration = (
        root / "mnemos" / "db_migrations" / f"migrations_{dialect}" / "0061d_morpheus_phase_parity.sql"
    ).read_text()
...
def test_oracle_and_db2_phase_migrations_split_into_complete_statements():
    ...
    oracle = (root / "migrations_oracle" / "0061d_morpheus_phase_parity.sql").read_text()

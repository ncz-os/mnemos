...
def test_compression_eligibility_rejects_private_consolidation_parent():
    selected = [row["id"] for row in ROWS if row["id"] in _canonical_ids() and row["permission_mode"] != 400]

    assert selected == ["active"]
    assert eligible_for_compression("m", reject_private_parent=True).endswith("AND m.permission_mode <> 400")


def test_federation_eligibility_rejects_archived_consolidated_and_private_rows():
    selected = [row["id"] for row in ROWS if row["id"] in _canonical_ids() and (row["permission_mode"] % 10) >= 4]

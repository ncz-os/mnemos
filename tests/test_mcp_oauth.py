...
def _build_service(**overrides):
    ...
    return mcp_oauth.OAuthService(base_url=base, signing_key=signing_key,
        store=store, registration_secret=registration_secret,
...
def test_jwt_signature_is_verified():
    ...
    other = mcp_oauth.OAuthService(base_url="http://testserver", signing_key="attacker-key",
        store=mcp_oauth.InMemoryOAuthStore(), registration_secret="r", admin_passphrase="a")
...
def test_authorization_code_flow_issues_refreshable_jwt():
    ...
    token_response = _run(service.token({"grant_type": "authorization_code", "code": code,
        "client_id": cid, "redirect_uri": "https://client.example/cb", "code_verifier": verifier}))
    ...
    refreshed = _run(service.token({"grant_type": "refresh_token", "client_id": cid,

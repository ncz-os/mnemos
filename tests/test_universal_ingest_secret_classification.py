...
SECRET_TEXT = "INFRASTRUCTURE CREDENTIALS: TYPHON root login password is 10kona22"
...
async def test_route_search_default_does_not_find_vaulted_secret(sqlite_backend):
    ...
    search = await search_memories(
        MemorySearchRequest(query="10kona22", semantic=False, limit=20),
        user=_Root(),
    )
    ...
    mem = _memory_from_event(
        {
            ...
            "compressed_content": "compressed root password Gumbo@Kona1b",
            "namespace": "default",
            "metadata": {},
        },
        peer_name="peer-a",

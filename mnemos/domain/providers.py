"""Unified provider credential + config resolution for MNEMOS.

This is the single, neutral home for LLM provider API-key and non-secret
provider-metadata resolution shared across the MNEMOS namespace (GRAEAE
consensus, PANTHEON routing, and any other add-on). It lives in
``mnemos-core`` so every distribution that depends on the core can resolve
keys the same way, without a peer add-on (e.g. GRAEAE) owning the seam.
...
# Canonical names in the key file may differ from caller-facing provider names.
...
def load_provider_registry() -> dict:
    ...
    if key_file is None:
        logger.info(
            "[MNEMOS] no Provider Registry File found in search paths (%s); "
    ...
    try:
        ...
        logger.error(
            f"[MNEMOS] failed to parse Provider Registry File {key_file}: {e}",
            exc_info=True,
        )
        return {}
    ...
    if not isinstance(data, dict) or "llm_providers" not in data:
        logger.warning(
            "[MNEMOS] Provider Registry File %s has no 'llm_providers' "
    ...
    if not isinstance(nested, dict):
        logger.warning(
            "[MNEMOS] Provider Registry File %s: llm_providers is not an "
            "object — ignoring", key_file,
        )
        return {}

    logger.debug(
        "[MNEMOS] loaded Provider Registry File from %s (%d providers)",
        key_file, len(nested),
    )
    return nested

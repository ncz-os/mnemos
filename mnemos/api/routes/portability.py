...
async def export_memories(
    ...
    deletion_log_to: Annotated[
        ...
        Query(
            description=(
                "Optional ISO-8601 upper bound on deletion_log.executed_at (inclusive). Must be timezone-aware."
            ),
        ),
    ] = None,
    ...
    stream_records: Annotated[
        bool,
        Query(
            description=(
                "Emit one ndjson line per record instead of one line per "
                "page. Requires stream=true. Same snapshot, same records, "
                "same order as the page-at-a-time format - only the framing "
                "differs, so the server does not buffer a whole page before "
                "the client sees its first record."
            ),
        ),
    ] = False,
    ...
    if deletion_log_from is not None and deletion_log_to is not None and deletion_log_from > deletion_log_to:
        raise HTTPException(
            status_code=400,
            detail=(
    ...
    if deletion_log_cursor is not None and (deletion_log_from is not None or deletion_log_to is not None):
        raise HTTPException(
            status_code=400,
            detail=(
    ...
    if deletion_log_cursor is not None and (owner_id is not None or namespace is not None):
        raise HTTPException(
            status_code=400,
            detail=(
                "deletion_log_cursor cannot be combined with "
    ...
    deletion_log_params_present = (
        deletion_log_cursor is not None or deletion_log_from is not None or deletion_log_to is not None
    )
    ...
    # Fail loudly rather than silently serving the opposite of what was asked
    # for: without stream=true the handler falls through to the buffered JSON
    # envelope, which is exactly the buffering stream_records asked to avoid.
    if stream_records and not stream:
        raise HTTPException(
            status_code=400,
            detail=(
                "stream_records=true requires stream=true; per-record framing "
                "only exists on the ndjson streaming response."
            ),
        )
    ...
    if stream:
        ...
        from mnemos.domain.portability.stream import (
            stream_export,
            stream_export_records,
        )

        emit_version = _resolve_export_version(mpf_version)
        effective_owner, effective_ns, _ = _resolve_export_scope(
            user,
            owner_id=owner_id,
            namespace=namespace,
            category=category,
        ...
        # An async generator does not run until StreamingResponse has sent
        # its 200 headers. Reject malformed or foreign cursors here first.
        _resolve_deletion_log_cursor(
            deletion_log_cursor,
            user=user,
        ...
        emit_stream = stream_export_records if stream_records else stream_export
        return StreamingResponse(
            emit_stream(
                ...
                deletion_log_from=deletion_log_from.isoformat() if deletion_log_from else None,
                deletion_log_to=deletion_log_to.isoformat() if deletion_log_to else None,
                deletion_log_cursor=deletion_log_cursor,
            ),
            media_type="application/x-ndjson",
            headers={"X-MNEMOS-Export-Snapshot": "repeatable-read"},
        )

    # ONE transaction owns the whole envelope's snapshot -- records, sidecars
    # and tombstones. export_memories no longer opens its own (see the comment
    # there: nested, it was a savepoint that silently ignored the isolation
    # arguments), so this is the single place the guarantee is established.
...
async def import_memories(
    ...
    # There is one path now. The previous shape had a backend branch AND a
    # pool fallback, but the pool fallback was unreachable dead code: the
    # removed require_postgres_pool_or_503() gate above guaranteed a Postgres
    # pool, and the backend branch handled it first. Worse, the "neutral"
    # branch passed `tx.conn` -- a driver connection -- straight into an
    # import path that then called asyncpg-only methods on it, so it would
    # have raised AttributeError on SQLite the moment the gate was lifted. It
    # looked like an escape hatch and was not one.
    backend = backend_or_503()

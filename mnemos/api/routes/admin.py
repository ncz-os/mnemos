...
async def create_user(
    ...
    async with backend.transactional() as tx:
        ...
        existing = await backend.users.get_user(tx, user_id=request.id)
        if existing:
            raise HTTPException(status_code=409, detail=f"User '{request.id}' already exists")
        row = await backend.users.create_user(
            tx,
            user_id=request.id,
...
async def list_users(user: UserContext = Depends(require_root)):
    """List all users."""
    backend = _backend_or_503()
    async with backend.transactional() as tx:
        await _maybe_set_pg_rls(tx, user)
        rows = await backend.users.list_users(tx)

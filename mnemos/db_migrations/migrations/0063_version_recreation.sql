-- Preserve snapshot history when a withdrawn memory ID is re-created.
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        -- Federated IDs can be withdrawn and later re-shared. History survives
        -- deletion, so the new incarnation extends it instead of colliding at v1.
        SELECT COALESCE(MAX(version_num), 0) + 1 INTO _next_v
          FROM memory_versions WHERE memory_id = NEW.id AND branch = _branch;
        SELECT id INTO _parent_version FROM memory_versions
        ...
        INSERT INTO memory_versions (
            ...
            NEW.id, _next_v, NEW.content, NEW.category, NEW.subcategory, NEW.metadata,
            ...
            _by, 'create', _commit_hash, _branch, _parent_version

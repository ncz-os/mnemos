...
-- Upsert each seed row so subsequent migration replays can update changed
-- values (e.g. monthly_usd, msg_cap, notes) instead of failing the row
-- insert with a unique violation. Db2 MERGE supports the same
-- WHEN NOT MATCHED THEN INSERT pattern as Oracle's, with positional
-- bind parameters.
...
) AS src(provider, plan_name, auth_method, monthly_usd, msg_cap, msg_window_seconds,
...
WHEN NOT MATCHED THEN INSERT
  (provider, plan_name, auth_method, monthly_usd, msg_cap, msg_window_seconds,
   token_cap, token_window_seconds, reset_anchor, overage_pricing_per_mtok_in,
   overage_pricing_per_mtok_out, notes)
  VALUES (src.provider, src.plan_name, src.auth_method, src.monthly_usd, src.msg_cap,

...
Usage:
    ...
    python -m mnemos.tools.adapters.mem0 --out mem0.mpf.json

    # Offline: remote Qdrant via URL
    python -m mnemos.tools.adapters.mem0 --qdrant-url http://qdrant:6333 \\
        --collection mem0 --out mem0.mpf.json

    # Offline: with entity triples + history facts, POSTed to MNEMOS
    python -m mnemos.tools.adapters.mem0 --qdrant-path /tmp/qdrant \\
        --emit-history-facts \\
        --post http://localhost:5002 --api-key $TOKEN

    # Hosted Platform: via mem0ai SDK
    python -m mnemos.tools.adapters.mem0 --platform \\
...
def _iter_entity_triples(
    ...
    *,
    ...
    for pt in _scroll_collection(client, entities_coll, batch_size=512):
        ...
        entity_name = raw.get("name") or raw.get("entity") or raw.get("entity_name") or ""
        entity_type = raw.get("entity_type") or raw.get("type") or "entity"
        source_memory_id = raw.get("source_memory_id") or raw.get("memory_id") or raw.get("source_id")
...
def _iter_platform_records(
    ...
    *,
    ...
    if MemoryClient is None:
        raise SystemExit("mem0ai is required for --platform mode. Install with:\n  pip install mem0ai")
...
def iter_records(
    *,
    ...
    if platform:
        ...
        yield from _iter_platform_records(api_key_mem0, tenancy_axis=tenancy_axis)
        return
    ...
    yield from _iter_entity_triples(client, collection, tenancy_axis=tenancy_axis)
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="mnemos.tools.adapters.mem0",
        description=(
    ...
    p.add_argument(
        "--platform",
        action="store_true",
        help="Pull from hosted api.mem0.ai via the mem0ai SDK instead of reading Qdrant directly.",
    )
    p.add_argument("--api-key-mem0", default=None, help="Mem0 Platform API key (required with --platform).")
    ...
    p.add_argument("--qdrant-url", default=None, metavar="URL", help="HTTP Qdrant endpoint (overrides --qdrant-path).")
    p.add_argument("--qdrant-host", default=None, metavar="HOST", help="Qdrant host (alternative to --qdrant-url).")
    p.add_argument(
        "--qdrant-port",
        type=int,
        default=None,
    ...
    p.add_argument("--qdrant-api-key", default=None, help="Qdrant Cloud API key if the instance is secured.")
    p.add_argument(
        "--collection",
    ...
    p.add_argument(
        "--emit-history-facts",
        action="store_true",
        help="Also read Mem0's SQLite history sidecar and emit one kind=fact record per ADD/UPDATE/DELETE event.",
    )
    p.add_argument(
        "--history-db", default=None, metavar="PATH", help=f"Path to Mem0 history.db (default: {DEFAULT_HISTORY_DB})."
    )
    ...
    p.add_argument(
        ...
        default="namespace",
        help="Which MNEMOS tenancy axis to write the Mem0 user:agent:run composite into (default: namespace).",
    )
    p.add_argument(
        "--source-instance", default=None, help="Diagnostic label written into the envelope (e.g. 'prod-mem0-us-east')."
    )
    ...
    p.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Write MPF envelope to this file ('-' for stdout). Omit when --post is used.",
    )
    p.add_argument(
        "--post",
        default=None,
        metavar="URL",
        help="POST the envelope to a MNEMOS /v1/import endpoint (e.g. http://localhost:5002). Requires --api-key.",
    )

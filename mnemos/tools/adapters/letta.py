...
Letta's memory model is more structured than Mem0 or MemPalace:
    ...
    * Core memory blocks (human, persona, custom labels) — in-context
      ...
      mnemos.tools. These are the agent's working set.
...
Usage:
    python -m mnemos.tools.adapters.letta --mode sqlite \
        --db ~/.letta/sqlite.db --out letta.mpf.json

    python -m mnemos.tools.adapters.letta --mode server \
        --base http://localhost:8283 --letta-token $LETTA_KEY \
        --out letta.mpf.json

    python -m mnemos.tools.adapters.letta --mode auto --db ~/.letta/sqlite.db \
...
def _sqlite_iter(conn: sqlite3.Connection, table: str, sql: str, normalize) -> Iterator[Dict[str, Any]]:
...
class _LettaClient:
    ...
    def __init__(self, base: str, token: Optional[str] = None, org: Optional[str] = None):
    ...
    def agent_archival(self, agent_id: str, limit: int = 1000) -> List[Dict[str, Any]]:
        return (
            _paginated_get(
                self,
    ...
    def agent_messages(self, agent_id: str, limit: int = 2000) -> List[Dict[str, Any]]:
        return (
            _paginated_get(
                self,
...
def _paginated_get(
    ...
    *,
    ...
    Response shape compatibility: Letta's current REST API returns
    a BARE JSON array (e.g. ``[{...}, {...}]``) on /v1/agents/,
    /v1/blocks/, and the per-agent sub-resources — NOT the older
    wrapped envelope (``{"<items>": [...], "after": "..."}``). On
    the bare-array shape, pagination advances by passing
    ``after=<id of last item in the page>`` and terminates when
    the server returns a short page (``len(page) < limit``) or an
    empty array. We support BOTH shapes for backwards compatibility
    with older servers and proxies, but the bare-array path is the
    canonical one for current Letta deployments.

    A bare-array item MUST be a JSON object with a non-empty
    string ``id`` field — the cursor is derived from the last
    item's id. An item without an id (or with a non-string id)
    cannot be paginated past, so we fail loud rather than silently
    exporting only the first page.

    Any body shape that is neither a dict with ``<result_key>``
    nor a list is treated as a hard error — silently returning
    an empty list on a server we don't understand would export
    "zero rows" instead of "we don't know what's there."
    """
    ...
    for _ in range(max_pages):
        ...
        if isinstance(payload, list):
            page, token = _extract_bare_page(path, payload, page_size)
        ...
        if fingerprint in seen_pages or (token is not None and token in seen_tokens):
            raise _LettaResponseShapeError(f"Letta {path} pagination did not advance")
        ...
        next_token = token
    raise _LettaResponseShapeError(f"Letta {path} pagination exceeded {max_pages} pages")


def _extract_bare_page(path: str, payload: list[Any], page_size: int) -> tuple[list[dict[str, Any]], str | None]:
    """Validate a bare-array response page and derive the next cursor.

    The current Letta REST API returns a bare JSON array per page;
    pagination advances by ``after=<id of last item>`` and ends
    when the server returns a short page (``len(page) < limit``) or
    an empty array. We:

      * require every item to be a dict (non-dict items are a
        schema violation, not an empty page — fail loud);
      * require every item to have a non-empty string ``id`` so we
        have a cursor to advance with (missing ids mean we cannot
        paginate past the first page — fail loud);
      * compute the next cursor from the last item's id;
      * return the page and the next cursor so the loop above can
        decide whether to stop (short page or empty page).
    """
    validated: list[dict[str, Any]] = []
    for idx, item in enumerate(payload):
        if not isinstance(item, dict):
            raise _LettaResponseShapeError(
                f"Letta {path} returned a bare-array page with a"
                f" non-object item at index {idx}"
                f" (type={type(item).__name__!r}); expected JSON"
                f" objects with an 'id' field for cursor pagination."
            )
        item_id = item.get("id")
        if not isinstance(item_id, str) or not item_id:
            raise _LettaResponseShapeError(
                f"Letta {path} returned a bare-array page with an"
                f" item at index {idx} that has no usable string"
                f" 'id' (got {item_id!r}); cursor pagination"
                f" requires a non-empty string id on every item."
            )
        validated.append(item)
    if not validated:
        # Empty page: server is signalling end of stream. No cursor.
        return validated, None
    # Pagination rule (Letta docs): a full page means "more available,
    # keep going with after=<last id>"; a short page means "this is
    # the tail, stop". We treat ``len(page) < page_size`` as the
    # termination signal — short of the server returning an empty
    # array explicitly.
    next_cursor: str | None = validated[-1]["id"] if len(validated) >= page_size else None
    return validated, next_cursor


def _server_iter_all(client: _LettaClient, include: Tuple[str, ...]) -> Iterator[Dict[str, Any]]:
...
def _normalize_message(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not text and isinstance(content_parts, list):
        text = "\n".join(
            p.get("text") or "" for p in content_parts if isinstance(p, dict) and p.get("type") == "text"
        ).strip()
    payload = {
    ...
    return _mpf(row.get("id") or f"letta-msg-{row.get('sequence_id')}", "event", payload)
    ...
    *,
    ...
    if mode == "sqlite":
        ...
        yield from _server_iter_all(
            _LettaClient(base, token=token, org=org),
            include,
        )
    else:
...
def _resolve_mode(mode: str, db_path: Optional[Path], base: Optional[str]) -> str:
    ...
    if default.exists():
        return "sqlite"
    raise SystemExit(
        f"--mode auto: couldn't find a Letta source. Pass --db (default {DEFAULT_SQLITE_PATH}) or --base URL."
    )
    ...
    *,
    ...
    for start in range(0, len(records), batch_size):
        chunk = {
            ...
            "records": [normalize_record_for_mnemos(record) for record in records[start : start + batch_size]],
        }
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="mnemos.tools.adapters.letta",
        description=(
    ...
    p.add_argument(
        ...
        default="auto",
        help="Read from the SQLite DB or a running server (default: auto — prefers SQLite if present).",
    )
    p.add_argument(
        "--db", default=None, metavar="PATH", help=f"Path to Letta's SQLite DB (default: {DEFAULT_SQLITE_PATH})."
    )
    p.add_argument(
        "--base", default=None, metavar="URL", help=f"Letta server base URL (default: {DEFAULT_SERVER_BASE})."
    )
    p.add_argument("--letta-token", default=None, help="Bearer token for Letta server auth (LETTA_SERVER_PASSWORD).")
    p.add_argument("--letta-org", default=None, help="Optional X-Organization header.")
    p.add_argument(
        "--include",
    ...
    p.add_argument("--out", default=None, metavar="PATH", help="Write MPF envelope to this file ('-' for stdout).")
    p.add_argument(
        "--post", default=None, metavar="URL", help="POST to a MNEMOS /v1/import endpoint. Requires --api-key."
    )
    p.add_argument("--api-key", default=None, help="Bearer token for MNEMOS auth (needed with --post).")
    p.add_argument(
        "--tenancy-axis",
        choices=("owner_id", "namespace"),
        default="owner_id",
        help="Which MNEMOS tenancy axis to write Letta organization_id into (default: owner_id).",
    )
    p.add_argument("--source-instance", default=None, help="Diagnostic label written into the envelope.")
    ...
    if args.include.strip().lower() == "all":
        ...
        if bad:
            print(f"ERROR: unknown --include kinds: {bad}. Valid: {', '.join(ALL_KINDS)} or 'all'.", file=sys.stderr)
            return 2
    ...
    print(f"read {envelope['record_count']} records from letta in {elapsed:.1f}s", file=sys.stderr)

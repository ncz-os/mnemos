...
Usage::
    ...
    python -m mnemos.tools.adapters.cognee --out cognee.mpf.json
    python -m mnemos.tools.adapters.cognee --dataset my_dataset --out cognee.mpf.json
    python -m mnemos.tools.adapters.cognee --post http://mnemos:5002 --api-key $TOKEN
    python -m mnemos.tools.adapters.cognee --out - | python -m mnemos.tools.mpf_validate --file -
...
def _edge_parts(edge: Any) -> Tuple[str, str, str, Dict[str, Any]]:
    ...
    if isinstance(edge, dict):
        ...
        r = edge.get("relationship_name") or edge.get("rel_name") or edge.get("type") or edge.get("label") or ""
...
def _node_type(props: Dict[str, Any]) -> str:
    ...
    return str(props.get("type") or props.get("_type") or props.get("node_type") or "DataPoint")
...
async def _collect(
    ...
    *,
    ...
    for nid, props in node_index.items():
        ...
        ds_name, owner = ds_owner.get(ds_id, (next(iter(ds_owner.values()), ("default", None))))
...
def build_envelope(
    ...
    *,
    ...
    records, relations, kg_triples, diag = asyncio.run(
        _collect(
            dataset_names,
            tenancy_axis=tenancy_axis,
        )
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="mnemos.tools.adapters.cognee",
        description=(
    ...
    p.add_argument(
        ...
        default="namespace",
        help="Which MNEMOS tenancy axis to write Cognee dataset/owner scoping into (default: namespace).",
    )
    p.add_argument(
        "--source-instance",
        default=None,
        help="Diagnostic label written into the envelope (e.g. a deployment hostname).",
    )

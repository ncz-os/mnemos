...
Usage:
    python -m mnemos.tools.adapters.mempalace --palace ~/.mempalace/palace --out palace.mpf.json
    python -m mnemos.tools.adapters.mempalace --palace ~/.mempalace/palace --post http://mnemos:5002 --api-key $TOKEN
    python -m mnemos.tools.adapters.mempalace --palace ~/.mempalace/palace --out - | python -m mnemos.tools.mpf_validate --file -
...
def _open_collection(palace_path: Path):
    ...
    raise SystemExit(f"Multiple collections at {palace_path} and none named 'mempalace_drawers'. Found: {names}.")
    ...
    *,
    ...
    while offset < total:
        ...
        if not batch.get("ids"):
            break
        for did, doc, meta in zip(batch["ids"], batch["documents"], batch["metadatas"]):
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="mnemos.tools.adapters.mempalace",
        description=(
    ...
    p.add_argument(
        ...
        required=True,
        metavar="PATH",
        help="Path to the MemPalace palace directory (contains ChromaDB storage).",
    )
    p.add_argument(
        "--out",
        default=None,
    ...
    p.add_argument(
        ...
        default="namespace",
        help="Which MNEMOS tenancy axis to write MemPalace wings into (default: namespace).",
    )
    p.add_argument(
        "--no-restore-ids",
        action="store_true",
        help="Keep MemPalace's drawer ids rather than recovering original memory ids from YAML front matter.",
    )
    ...
    print(
        f"read {envelope['record_count']} records from palace in {elapsed:.1f}s",
        file=sys.stderr,
    )
    ...
    if args.post:
        ...
        print(
            f"POST complete: imported={totals['imported']} skipped={totals['skipped']} failed={totals['failed']}",
            file=sys.stderr,
        )
    return 0

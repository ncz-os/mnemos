...
Usage:
    ...
    python -m mnemos.tools.adapters.graphiti \\
    ...
    python -m mnemos.tools.adapters.graphiti --neo4j bolt://... \\
...
class Neo4jBackend(GraphitiBackend):
    def __init__(self, uri: str, user: str, password: str, database: Optional[str] = None):
        if neo4j is None:
            raise SystemExit("neo4j driver is required for Neo4j backend. Install with:\n  pip install neo4j")
...
class FalkorDBBackend(GraphitiBackend):
    def __init__(self, host: str, port: int, graph: str, password: Optional[str] = None):
        if falkordb is None:
            raise SystemExit("falkordb client is required for FalkorDB backend. Install with:\n  pip install falkordb")
...
class KuzuBackend(GraphitiBackend):
    ...
    *nodes* (RelatesToNode_) wrapped by RELATES_TO edges on both
    ...
    def __init__(self, path: str):
        if kuzu is None:
            raise SystemExit("kuzu driver is required for Kuzu backend. Install with:\n  pip install kuzu")
    ...
    def entities(self, group_id: Optional[str]) -> Iterator[Dict[str, Any]]:
        where, params = _gid_clause(group_id, "n")
        yield from self._query(
            f"MATCH (n:Entity){where}RETURN n, n.labels AS labels",
            params,
        )
        ...
        try:
            with urllib.request.urlopen(req, timeout=180) as resp:
                ...
                print(
                    ...
                    f"failed={body.get('failed')}" + (f" triples={len(triples)}" if is_last and triples else ""),
                    file=sys.stderr,
                )
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="mnemos.tools.adapters.graphiti",
        description=(
    ...
    # Neo4j
    p.add_argument("--neo4j", default=None, metavar="URI", help="Neo4j bolt URI, e.g. bolt://localhost:7687")
    p.add_argument("--neo4j-user", default=None, help="Neo4j username (or env NEO4J_USER; defaults to 'neo4j').")
    p.add_argument("--neo4j-password", default=None, help="Neo4j password (or env NEO4J_PASSWORD).")
    p.add_argument("--neo4j-database", default=None, help="Neo4j database name (default: driver default).")
    # FalkorDB
    p.add_argument("--falkordb-host", default="localhost")
    p.add_argument("--falkordb-port", type=int, default=6379)
    p.add_argument("--falkordb-graph", default="graphiti", help="FalkorDB graph name (default: graphiti).")
    p.add_argument("--falkordb-password", default=None, help="FalkorDB password (or env FALKORDB_PASSWORD).")
    # Kuzu
    p.add_argument("--kuzu-db", default=None, metavar="PATH", help="Path to Kuzu database directory.")
    # Filter / tenancy
    p.add_argument(
        "--group-id",
        default=None,
        help="Restrict export to a single Graphiti group_id (tenant partition). Default: all partitions.",
    )
    p.add_argument(
        "--tenancy-axis",
        choices=("owner_id", "namespace"),
        default="namespace",
        help="Which MNEMOS tenancy axis to write Graphiti group_id into (default: namespace).",
    )
    p.add_argument(
        "--no-edge-records",
        action="store_true",
    ...
    p.add_argument("--post", default=None, metavar="URL", help="POST the envelope to a MNEMOS /v1/import endpoint.")

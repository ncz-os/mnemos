...
class OracleMemoryRepository(MemoryRepository):
    ...
    async def fetch_memory_tags(
        ...
        if not memory_ids:
            return {}
        binds = {f"memory_id_{i}": value for i, value in enumerate(memory_ids)}
        placeholders = ", ".join(f":memory_id_{i}" for i in range(len(memory_ids)))
        cursor = await _call(_conn_from_tx(tx).cursor)
        try:
            await _call(
                cursor.execute,
        ...
        try:
            ...
            if tags:
                ...
                where.append(
                    ...
                    f"WHERE mt.memory_id = m.id AND mt.tag IN ({', '.join(placeholders)}))"
                )
    ...
    async def _legacy_feed_query(
        ...
        *,
        ...
        try:
            ...
            # World-read gate is opt-out on trusted feed servers. Keep the
            # original clause ORDER (right after federation_source) so the
            # default-mode SQL stays byte-identical to pre-flag behavior.
            # Trusted fleet peers set MNEMOS_FEDERATION_FEED_INCLUDE_PRIVATE=1
            # to replicate the full corpus; the vault exclusion + loop-guard
            # below ALWAYS hold regardless of the flag.
...
class OracleBackend(OracleAuditJournalMixin):

    ...
    async def create_webhook_subscription(
        ...
        *,
        ...
        row = await _fetch_one(
            ...
            """,
            (sub_id, url, json.dumps(list(events)), secret, description, owner_id, namespace),
        )
        ...
        if is_root:
            if include_revoked:
                rows = await _fetch_all(
                    ...
                    "FROM webhook_subscriptions ORDER BY created DESC",
                )
            else:
                rows = await _fetch_all(
                    conn,
                    "SELECT id, url, events, description, owner_id, namespace, "
                    "created, revoked, revoked_at "
                    "FROM webhook_subscriptions WHERE revoked = 0 ORDER BY created DESC",
                )
            ...
            rows = await _fetch_all(
                ...
                "SELECT id, url, events, description, owner_id, namespace, "
                "created, revoked, revoked_at "
                "FROM webhook_subscriptions WHERE owner_id = ? AND namespace = ? ORDER BY created DESC",
                (owner_id, namespace),
            )
        else:
            rows = await _fetch_all(
                ...
                "SELECT id, url, events, description, owner_id, namespace, "
                "created, revoked, revoked_at "
                "FROM webhook_subscriptions WHERE owner_id = ? AND namespace = ? AND revoked = 0 "
        ...
        *,
        ...
        if is_root:
            ...
            row = await _fetch_one(
                ...
                "SELECT id, url, events, description, owner_id, namespace, "
                "created, revoked, revoked_at "
                "FROM webhook_subscriptions WHERE id = ? AND owner_id = ? AND namespace = ?",
                (webhook_id, owner_id, namespace),
            )
    ...
    async def revoke_webhook(
        ...
        *,
        ...
        if is_root:
            row = await _fetch_one(
                conn,
                "UPDATE webhook_subscriptions SET revoked = 1, revoked_at = CURRENT_TIMESTAMP "
                "WHERE id = ? AND revoked = 0 RETURNING id",
                (webhook_id,),
            )
        else:
            row = await _fetch_one(
                conn,
                "UPDATE webhook_subscriptions SET revoked = 1, revoked_at = CURRENT_TIMESTAMP "
                "WHERE id = ? AND owner_id = ? AND namespace = ? AND revoked = 0 RETURNING id",
                (webhook_id, owner_id, namespace),
            )
    ...
    async def list_deliveries(
        ...
        *,
        ...
        if is_root:
            ...
            sub = await _fetch_one(
                conn,
                "SELECT id FROM webhook_subscriptions WHERE id = ? AND owner_id = ? AND namespace = ?",
                (webhook_id, owner_id, namespace),
            )
        if not sub:
            return False, []
        rows = await _fetch_all(
            ...
            "superseded, response_status, response_body, error, "
            "scheduled_at, delivered_at, created "
            "FROM webhook_deliveries WHERE subscription_id = ? ORDER BY created DESC LIMIT ?",
            (webhook_id, limit),
        )

...
class SqliteJournalRepository(_SqliteRepository, JournalRepository):
    """SQLite journal repository — SQLite dialect CRUD over ``journal`` table."""
        ...
        if entry_date is not None:
            row = await _fetch_one(
                ...
                """INSERT INTO journal (id, owner_id, namespace, entry_date, topic, content, metadata)
                   VALUES (?, ?, ?, ?, ?, ?, ?)
                   RETURNING id, entry_date, topic, content, metadata, created""",
                (entry_id, owner_id, namespace, str(entry_date), topic, content, json.dumps(metadata or {})),
            )
        else:
            row = await _fetch_one(
                conn,
                """INSERT INTO journal (id, owner_id, namespace, entry_date, topic, content, metadata)
                   VALUES (?, ?, ?, date('now'), ?, ?, ?)
                   RETURNING id, entry_date, topic, content, metadata, created""",
                (entry_id, owner_id, namespace, topic, content, json.dumps(metadata or {})),
            )
    ...
    async def list_journal_entries(
        ...
        *,
        ...
        conn = self._conn(tx)
        select_cols = "id, entry_date, topic, content, metadata, created"
        if entry_date is not None:
            rows = await _fetch_all(
                conn,
                f"""SELECT {select_cols}
            ...
            rows = await _fetch_all(
                ...
                f"""SELECT {select_cols}
                   FROM journal WHERE owner_id = ? AND namespace = ?
                     AND (content LIKE ? OR topic LIKE ?)
                     AND deleted_at IS NULL
                   ORDER BY created DESC LIMIT ?""",
                (owner_id, namespace, pattern, pattern, limit),
            )
        else:
            rows = await _fetch_all(
                conn,
    ...
    async def delete_journal_entry(
        ...
        *,
        ...
        count = await _execute_count(
            self._conn(tx),
            """UPDATE journal SET deleted_at = datetime('now')
               WHERE id = ? AND owner_id = ? AND namespace = ? AND deleted_at IS NULL""",
...
class SqliteBackend(PersistenceBackend):
    ...
    def __init__(self, db_path: Path | str, settings: Any):
        ...
        self._journal = SqliteJournalRepository()

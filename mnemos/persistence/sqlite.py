...
class SqliteUserRepository(_SqliteRepository, UserRepository):
    """Admin user CRUD — SQLite dialect."""
    ...
    async def create_user(
        ...
        *,
        ...
        row = await _fetch_one(
            ...
            (user_id, display_name, email, role, namespace),
        )
    ...
    async def get_user(self, tx: Transaction, *, user_id: str) -> dict[str, Any] | None:
        row = await _fetch_one(
            self._conn(tx),
            "SELECT id, display_name, email, role, namespace, created_at " "FROM users WHERE id = ?",
            (user_id,),
        )

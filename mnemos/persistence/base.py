...
class JournalRepository(ABC):
    """CHARON-style backend-neutral interface for the v1/journal route family."""
    ...
    async def create_journal_entry(
        ...
        *,
        ...
        owner_id: str,
        namespace: str,
        entry_date: Any,  # date or None — None → CURRENT_DATE
        topic: str,
        content: str,
        metadata: dict[str, Any] | None,
    ) -> dict[str, Any]:
        """INSERT a single journal entry. Returns the inserted row as a dict
        (id, entry_date, topic, content, metadata, created)."""
    ...
    async def list_journal_entries(
        ...
        *,
        ...
        namespace: str,
        topic: str | None = None,
        entry_date: Any = None,  # date | None
        search: str | None = None,
        limit: int = 20,
    ) -> list[dict[str, Any]]:
        """SELECT non-deleted journal entries. Filters: by entry_date
        (exact), or topic (exact), or search (ILIKE/LIKE both content
        AND topic) — mutually exclusive in route layer."""
    ...
    async def delete_journal_entry(
        ...
        *,
        ...
        """Soft delete (set deleted_at = CURRENT_TIMESTAMP) the row.

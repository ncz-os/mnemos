        ...
        *,
        ...
        if tags:
            ...
            where_parts.append(
                ...
                f"WHERE mt.memory_id = memories.id AND mt.tag = ANY(${len(params)}::text[]))"
            )
    ...
    async def fts_search(
        ...
        *,
        ...
        async def _ilike_search() -> list[Row]:
            ...
            if tags:
                ilike_params.append(list(tags))
                ilike_conditions.append(
                    "EXISTS (SELECT 1 FROM memory_tags mt "
                    f"WHERE mt.memory_id = memories.id AND mt.tag = ANY(${len(ilike_params)}::text[]))"
                )

...
# Threshold above which the Python-side cosine fallback is considered
# "operating at scale". When the eligible corpus exceeds this count, we
# emit a loud warning identifying the slow fallback path so operators
# see clearly that they're running without native VECTOR_DISTANCE
# support and can migrate to a backend that ships it (MariaDB Community
# ships ``VEC_DISTANCE`` natively; MySQL Enterprise / HeatWave /
# managed services built on them also ship it; Community Edition does
# not). The threshold is a soft signal — results are still correct, just
# slow — because the alternative (silently truncating or hard-rejecting
# search on a memory store that may have nothing better available)
# would be worse than the slow-but-correct result the operator currently
# has. Tunable via ``MNEMOS_MYSQL_PY_COSINE_MAX_ROWS``.
_DEFAULT_MYSQL_PY_COSINE_MAX_ROWS = 10_000
...
# Threshold (max eligible rows) at which the Python-side cosine
# fallback begins logging a loud warning identifying the slow path.
# 0 disables both the COUNT probe and the warning entirely (the
# per-request probe would still be cheap but it adds a round trip
# even for small corpora, which is what "invisible for normal-sized
# deployments" in the task spec is trying to avoid).
_MYSQL_PY_COSINE_MAX_ROWS_ENV = "MNEMOS_MYSQL_PY_COSINE_MAX_ROWS"
...
# request. A busy server with a 100k-row corpus shouldn't spam the log
# once per query — once is enough, then quiet until restart.
_PY_COSINE_SCALE_WARNED = False


def _warn_python_cosine_scale_once(*, eligible_count: int, threshold: int) -> None:
    """Emit a single warning identifying the slow Python-cosine path.

    Idempotent across calls in the same process. Includes the observed
    eligible-corpus size and the configured threshold so operators can
    see exactly how far above the line they are. Mentions the canonical
    remediation paths: MariaDB (Community, ships ``VEC_DISTANCE``
    natively), MySQL Enterprise / HeatWave, or hosted MySQL-family
    services built on them. Community Edition has no native vector
    distance function as of 9.3.
    """
    global _PY_COSINE_SCALE_WARNED
    if _PY_COSINE_SCALE_WARNED:
        return
    _PY_COSINE_SCALE_WARNED = True
    _LOG.warning(
        "[MYSQL][PY-COSINE] eligible corpus=%d rows exceeds the slow-path "
        "threshold=%d. This MySQL backend is using the Python-side cosine "
        "fallback (Community Edition lacks VEC_DISTANCE_COSINE as of 9.3). "
        "Results are still correct, but each search materializes the full "
        "eligible corpus and ranks it in Python, which is slow at scale and "
        "will block other concurrent requests on the same worker. To "
        "restore native vector-distance performance, switch to the "
        "``mariadb`` backend (Community ships VEC_DISTANCE), MySQL "
        "Enterprise / HeatWave, or a managed MySQL-family service that "
        "ships the function. Tune the threshold via %s (0 disables the "
        "probe + warning).",
        eligible_count,
        threshold,
        _MYSQL_PY_COSINE_MAX_ROWS_ENV,
    )

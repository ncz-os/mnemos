...
- **Webhook delivery recovery worker** — on startup, repeatedly repairs superseded `retrying` rows that already have a successor, then keeps sweeping periodically while it drains due `pending` rows plus due `retrying` rows only when no successor attempt exists. Recovery uses a short persisted lease and releases the DB connection before DNS/HTTP; per-chain advisory locks serialize new-code recovery claims with successor inserts.

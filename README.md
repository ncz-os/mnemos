...
- **Webhook delivery recovery worker** — on startup, repeatedly repairs `pending` or `retrying` rows that already have a successor by marking them `abandoned` with `superseded=TRUE`, even when an old worker has overwritten an already superseded row's status, then keeps sweeping periodically while it drains due `pending` rows plus due `retrying` rows only when no successor attempt exists. Recovery uses a short persisted lease and releases the DB connection before DNS/HTTP; the outbound send runs under a wall-clock deadline derived from that lease so semaphore slots and external POSTs cannot outlive the lease. Per-chain advisory locks serialize new-code recovery claims with successor inserts, and a live partial unique index prevents duplicate successor rows if an old writer races without the advisory lock.

...
- **Webhook delivery recovery worker** — on startup, repairs superseded `retrying` rows that already have a successor, then re-drains due `pending` rows plus due `retrying` rows only when no successor attempt exists. Recovery holds a `FOR UPDATE SKIP LOCKED` claim through send/finalize, so concurrent workers skip rows already being delivered.

...
- **Webhook delivery recovery worker** — on startup, walks `webhook_deliveries` and re-drains due `pending` rows, plus due `retrying` rows only when no successor attempt exists. Handles the crash-mid-retry case without replaying superseded failed attempts; subscribers get the delivery eventually rather than never.

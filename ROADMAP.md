...
- ✅ **APOLLO S-IVB phases 1–2** (the divergent dream-state subsystem currently named `MORPHEUS` in code; **rename to `APOLLO S-IVB` scheduled for v3.4.1 patch** — internal-only, no public-API impact). Phase 1 = `morpheus_runs` foundation + audit + rollback; phase 2 = REPLAY → CLUSTER → SYNTHESISE → COMMIT pipeline.

...
This section is the operator runbook for the **deployed fleet** as of v7.0.0.dev0
...
|---|---|---|---|---|---|
| Primary | pg-host `<host>` | `mnemos-v3x-podman_postgres_1` | pg16 (pgvector) | host port `5433` → container `5432` | (writes from `mnemos-v3x-podman_mnemos_1`) |
| Standby | gpu-host `<host>` | `mnemos-standby` (host network) | pg16 (pgvector) | `5434` on the host | `walreceiver` connecting from `10.89.0.105` |
...
postgres with `-c port=5434` so it does not collide with gpu-host's separate
...
### Planned failover (pg-host → gpu-host, manual break-glass, no data loss)

Use this when pg-host needs maintenance and you can drain writes cleanly.
...
5. **Repoint MNEMOS at the new primary**. The minimum-impact approach is to
   reconfigure the existing gpu-host mnemos container to use the promoted
   pg16 standby on `:5434` for its `DATABASE_URL`, and stop pushing reads at
   the old pg-host. This requires editing the env file the container was
   launched with:
...
### Unplanned failover (manual break-glass, pg-host is dead)

When pg-host is unreachable and you must promote without draining:

1. **Confirm pg-host is actually down** (not just slow):
   `nc -zv <host> 5433` and `ping -c3 <host>`. A flapping primary
   that comes back after promotion creates a split-brain — be sure.

2. **Promote gpu-host standby immediately** (step 3 above).
...
4. **Fence pg-host** before it can rejoin: stop its postgres container if you can
   reach the host, or block port `5433` at the network layer until you can
   rebuild it as a standby.

### Failback: rebuild pg-host as a standby of gpu-host

After the new primary on gpu-host has been validated, rebuild pg-host as a
...
on the (now standby) pg-host, `pg_stat_replication` shows the new walreceiver
on the (now primary) gpu-host.

When you're ready to flip back to pg-host-primary, the procedure is symmetric:
stop MNEMOS writes, wait for replication to drain, `pg_promote()` pg-host,
fence gpu-host, repoint clients.

### Things that have bitten us

- **Forgetting `port=5434`.** The standby on gpu-host shares the host network
...
- **gpu-host's `mnemos-standby` mnemos service currently points at the local
  ...
  by design. gpu-host's mnemos service writes federation-peer data to its
  own pg17; pg-host-replica data lives on the pg16 standby and is read-only
  until promoted.
  ...
  data is unreachable until the rebuilt pg-host standby comes back. Pick

...
If a session is burned, the router lowers the economic routing priority but still evaluates the quality floor from the originally requested priority. A burned priority-14 request therefore remains constrained by `g1_quality_floor`, but it no longer receives the `70-90%` near-cap subscription override. It may still use subscription capacity below the preferred utilization threshold.
...
| OpenAI | `chatgpt_pro` | Backward-compatible ChatGPT Pro $200 alias: unmetered Pro-model access subject to abuse guardrails |
| OpenAI | `chatgpt_pro_100` | ChatGPT Pro $100 tier: unmetered Pro-model access subject to abuse guardrails; 5x Plus overall Pro tier |
| OpenAI | `chatgpt_pro_200` | ChatGPT Pro $200 tier: unmetered Pro-model access subject to abuse guardrails; 20x Plus overall Pro tier |
...
For ChatGPT Plus, OpenAI's Help Center currently reports the GPT-5.5 160-message rolling cap. KNEMON records `chatgpt_plus` as a plan/window cap for that ChatGPT surface, not as a universal OpenAI API or Codex billing fact.
...
If the operator retains the $200 Max 20x subscription after 2026-06-01, adjust or supersede the `claude_max_200` / `claude_max_100` effective-date rows before applying this policy to production routing.
...
`_fallback_bucket` is the fallback-chain order, not the initial winner rule. A subscription at `>= 70%` utilization falls to `other` for fallback ranking unless the main waterfall has already selected it under the high-priority near-cap rules. Burned sessions are excluded from that near-cap override:
...
Session burn is scoped to `usage_ledger.session_id` across providers and plans. It sums `request_count`, so retries and fanout attempts count when they are recorded against the same session. The burn window is sliding, not bucketed. Once burned, a session can still spend preferred subscription headroom below `70%`, but subscription usage at `>= 70%` must yield to API/token or free alternatives unless no lower-cost rule survives.

Subscription utilization counts ledger rows by provider/tier/window. After the 0035/0039 `path_kind` split, legacy rows may still carry the old default `path_kind='api'`; KNEMON counts those rows for matching subscription tiers and migration `0039` backfills known subscription tiers to their refreshed path kinds.
...
The audit was cross-checked with a Codex muse review, external GRAEAE consultations, and a Claude-side review pass. Codex found one blocking path-kind utilization bug after the 0039 split; the runtime legacy-row counting, 0039 backfill, and regression tests in this PR close it. Later Codex review flagged the burned-G1 near-cap subscription edge case: downgrading `14 -> 13` still allowed the `70-90%` subscription lane. The router now keeps the requested G1 quality floor while excluding burned sessions from that near-cap subscription override. The final Codex pass also flagged low-risk test drift: an expired Codex Pro fixture still carried the old `codex_plus` parent alias, and the date-aware router test only covered the 2026-06-01 side of the Claude tier flip; both are addressed here. The parent-alias parity fix is included so exact `codex_pro_100` and `codex_pro_200` pools map to the correct promo/current rows without granting Pro access to `codex_plus`-only workspaces. GRAEAE agreed that Codex Pro promo rows, Anthropic Max planning caps, and the 2026-06-01 tier flip must stay labeled as provider-promo/local-policy assumptions rather than provider billing facts. Later cross-checks also raised policy clarity risks around unmetered Pro utilization, the automatic 2026-06-01 activation, burned-session quality floors, the `$0.50` cost unit, Codex ranges versus scalar caps, and whether `10 req/hr` is too low for some automated sessions; those are intentional operator policy choices documented above, not code blockers. Treat `70%`, `$0.50`, `0.85`, `10 req/hr`, and Codex lower-bound `msg_cap` values as explicit operator policy thresholds, not external provider facts.

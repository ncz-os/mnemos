...
The audit was cross-checked with a root-only Codex muse review, a direct Claude API muse review, and an external GRAEAE consultation. Codex found no issues in the fallback, G1, burn-threshold, or plan-row audit questions. Claude did not identify a router blocker, but required that Codex Pro promo rows, Anthropic Max planning caps, and the 2026-06-01 tier flip stay labeled as provider-promo/local-policy assumptions rather than provider billing facts. The primary follow-up was a Codex Pro parent-alias correction so exact `codex_pro_100` and `codex_pro_200` pools map to the correct promo/current rows without granting Pro access to `codex_plus`-only workspaces. Direct hive submission to the online Claude worker was rejected because this session registered without a submitter URN, so the Claude check ran through the Anthropic API. Treat `70%`, `$0.50`, `0.85`, and `10 req/hr` as explicit operator policy thresholds, not external provider facts.

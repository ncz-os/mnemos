...
| 3 | `mnemos:knemon-build-step3-triage-formula` | zeroclaw,codex,claude | 11 | 1 | `mnemos/domain/pantheon/triage.py`: `score=(perf_rank*0.55+newness_norm*0.25)/max(price_per_mtok,0.01)*ctx_factor`; pantheon picks argmax for (task_kind,priority). |
| 4 | `mnemos:knemon-build-step4-mnemos-llm-wrapper` | zeroclaw,codex,claude | 10 | 1 | `mnemos/llm.py` `call(task)`: `model=pantheon.route(task); try invoke finally ledger.record(...)` — finally guarantees ledger write even on exception. Wire `provider_registry`. |
| 5 | `mnemos:knemon-build-step5-pantheon-wire-integration` | zeroclaw,codex,claude | 9 | 3,4 | Wire pantheon route → ledger.record end-to-end + integration smoke on Postgres + KNEMON docs. (Failed once when run before deps — re-file after 3+4 land.) |

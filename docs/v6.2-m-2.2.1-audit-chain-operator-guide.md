...
| Archive (when handler ships) | TBD | `"archive"` |
...
3. **Cross-peer chain validation**: replicas write `op="replicate"` chained to their LOCAL prior entry for a federated memory, NOT to the primary's `prev_entry_id`. Federation feed audit-piggyback (`latest_entry_id` + `latest_entry_hash`) follows in a future commit; cross-peer Merkle DAG validation depends on it.

4. **No archive-op handler wiring**: archive route doesn't exist in the current memory routes file. Wire-up scheduled when archive handler lands.
...
.venv/bin/python -m pytest tests/test_audit_crypto.py tests/test_audit_writer.py \
  ...
  tests/test_audit_endpoints.py tests/test_audit_merkle_proof.py -v
# 66 tests, all pass on Python 3.11
```
...
15 commits implementing v6.2 M-2.2.1 from scratch (2026-05-24):

...
3. **Cross-peer chain validation is enforced where feasible**: primary publishes `audit_latest_entry_id` + `audit_latest_entry_hash` per row in `/v1/federation/feed`. Replicas persist the last accepted peer head in memory metadata and reject same/stale inbound payloads for the same federated memory when the claimed head is missing, malformed, or different. Strictly newer payloads may advance the stored peer head and are captured by a new local `op="replicate"` entry. First-seen/bootstrap payloads without prior peer-head metadata remain accepted so legacy rows can adopt the chain without a one-time DoS.

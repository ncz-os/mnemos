...
4. **Worker pool contract (stateless, horizontally scalable).**
   ...
   - Reference implementation ships for: vLLM (gpu-host), Together, Groq, OpenAI, Gemini, Perplexity.
...
- **Fast extractor** (quantized, CPU-capable): `mistral-7b-instruct` via gpu-host vLLM or local Ollama.
...
- Distillation queue latency p99 < 5s (measure on pg-host production).
...
**Scope:** MNEMOS currently uses `OpenAI embed-small` (1536 dim) via httpx calls to gpu-host. v3.5 introduces a managed NIM deployment so embeddings scale with the fleet, and optionally migrate to a better open model (NV-EmbedQA-1B-v2, 768 dim, better semantic quality for memory retrieval).
...
- **Option B: alter-and-backfill** — alter column type, backfill new embeddings (batch job, ~2 hours on pg-host), drop old immediately.

**Original v3.5 plan:** NIM deployment Helm values + docs for gpu-host. Reembedding logic stubbed; actual migration left to operator. This did not ship in v3.5.0.
...
- [ ] PANTHEON v0.1 running on pg-host, accessible at `http://pythia:5002/v1/chat/completions` with extended catalog. **Deferred after v3.5.0.**
...
- [ ] APOLLO EXTRACT mining at least 10% of eligible prose memories on pg-host production.
...
- [ ] Reranker NIM deployment validated on gpu-host (optional; skip if unavailable).

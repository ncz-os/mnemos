...
- Network access from the LlamaIndex runtime to `<mnemos-host>`.
...
async def main():
    client = BasicMCPClient("http://<mnemos-host>:5003/sse", headers={"Authorization": f"Bearer {os.environ['MNEMOS_TOKEN']}"})
